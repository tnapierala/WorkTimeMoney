  ...
  return (
    <SafeAreaView className={`flex-1 ${isDark ? 'bg-slate-900' : 'bg-gray-100'}`}>
      ...
      <View className={`flex-row items-center px-6 py-4 border-b ${isDark ? 'border-slate-800 bg-slate-900' : 'border-gray-200 bg-white'}`}>
        ...
        <Text className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('editEntryTitle')}</Text>
      </View>
      <View className="flex-1 justify-center p-6">
        <View className={`rounded-3xl p-6 border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100 shadow-xl'}`}>
          <Text className={`text-2xl font-extrabold mb-6 text-center ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('enterDataTitle')}</Text>
          <View className="mb-5">
            <Text className={`text-sm font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-700'}`}>{t('hours')}</Text>
            <TextInput
          ...
          <View className="mb-6">
            <Text className={`text-sm font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-700'}`}>{t('rateNet')}</Text>
            <TextInput
              className={`border rounded-xl p-4 text-base ${isDark ? 'bg-slate-900 border-slate-700 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`}
              keyboardType="numeric"
              value={netIncome}
